...
class ArticleModel extends BaseModel
{
    ...
    public function getLists($filter, $page = 1, $page_size)
    {
        ...
        $sql = "select * from sdmb_article a where $where $order limit " . $limit;
        $rs = $this->query($sql);
        return $rs;
    }
    ...
    public function getCounts($filter)
    {
        ...
        $sql = "
	        select count(1) as num from sdmb_article a where $where 
	        ";
        $rs = $this->query($sql);
        return $rs[0]['num'];
    }
    ...
    {
        $sql = "select max(a.sort) as max_sort from sdmb_article a";
        $rs = $this->query($sql);
