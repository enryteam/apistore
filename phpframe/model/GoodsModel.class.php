...
class GoodsModel extends BaseModel
{
    ...
    public function getLists($filter, $page = 1, $page_size, $get_count=false)
    {
        ...
        if ($get_count) {
            $sql = "select count(1) as num from sdmb_goods a where $where";
            $rs = $this->query($sql);
            return $rs[0]['num'];
        }
        $limit = ($page - 1) * $page_size . ',' . $page_size;
        $sql = "select * from sdmb_goods a where $where $order limit " . $limit;
        $rs = $this->query($sql);
        return $rs;
    }
    ...
    public function getCounts($filter)
    {
        ...
        $sql = "
	        select count(1) as num from sdmb_goods a where $where 
	        ";
        $rs = $this->query($sql);
        return $rs[0]['num'];
    }
    ...
    {
        $sql = "select max(a.sort) as max_sort from sdmb_goods a";
        $rs = $this->query($sql);
