 ...
 * showmessage('登录成功', array('默认跳转地址'=>'http://www.51daniu.cn'));
 *
 * @param string $msg
 *            提示信息
...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(), $setpages = 10)
{
    ...
    if ($num > $perpage) {
        ...
        if ($curr_page > 0) {
            if ($curr_page == 1) {
                $multipage .= ' <li class="paginate_button disabled"><a href="javascript:;" class="a1">' . L('previous') . '</a></li> <li class="paginate_button active"><a href="javascript:;">1</a></li>';
            } elseif ($curr_page > 6 && $more) {
                $multipage .= ' <li class="paginate_button"><a href="' . pageurl($urlrule, $curr_page - 1, $array) . '" class="a1">' . L('previous') . '</a></li> <li class="paginate_button"><a href="' . pageurl($urlrule, 1, $array) . '">1</a> </li><li class="paginate_button"><a href="javascript:;">...</a></li>';
            } else {
                $multipage .= ' <li class="paginate_button"><a href="' . pageurl($urlrule, $curr_page - 1, $array) . '" class="a1">' . L('previous') . '</a></li> <li class="paginate_button"><a href="' . pageurl($urlrule, 1, $array) . '">1</a></li>';
            }
        }
        for ($i = $from; $i <= $to; $i ++) {
            if ($i != $curr_page) {
                $multipage .= ' <li class="paginate_button"><a href="' . pageurl($urlrule, $i, $array) . '">' . $i . '</a></li>';
            } else {
                $multipage .= ' <li class="paginate_button active"><a href="javascript:;">'.$i.'</a></li>';
            }
        }
        if ($curr_page < $pages) {
            if ($curr_page < $pages - 5 && $more) {
                $multipage .= ' <li class="paginate_button"><a href="javascript:;">...</a></li><li class="paginate_button"><a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a></li> <li class="paginate_button"><a href="' . pageurl($urlrule, $curr_page + 1, $array) . '" class="a1">' . L('next') . '</a></li>';
            } else {
                $multipage .= ' <li class="paginate_button"><a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a></li> <li class="paginate_button"><a href="' . pageurl($urlrule, $curr_page + 1, $array) . '" class="a1">' . L('next') . '</a></li>';
            }
        } elseif ($curr_page == $pages) {
            $multipage .= ' <li class="paginate_button active"><a href="javascript:;">' . $pages . '</a></li> <li class="paginate_button disabled"><a href="javascript:;" class="a1">' . L('next') . '</a></li>';
        } else {
            $multipage .= ' <li class="paginate_button"><a href="' . pageurl($urlrule, $pages, $array) . '">' . $pages . '</a>/li> <li class="paginate_button"><a href="' . pageurl($urlrule, $curr_page + 1, $array) . '" class="a1">' . L('next') . '</a></li>';
        }
    }
    $multipage = '<div class="row"><div class="col-sm-3"><div class="dataTables_info">共'.$num.'条记录</div></div><div class="col-sm-9"><div class="dataTables_paginate paging_simple_numbers"><ul class="pagination">'.$multipage.'</ul></div></div></div>';
    return $multipage;
}
 ...
 * @return 分页
 */
function jbw_pages($num, $curr_page, $perpage = 5, $urlrule = '', $array = array())
{
    if (defined('URLRULE') && $urlrule == '') {
        $urlrule = URLRULE;
        $array = $GLOBALS['URL_ARRAY'];
...
function returnJsonp($result,$message,$data='',$page=''){
	if(!empty($page)){
		$json = array('code'=>$result,'message'=>$message,'data'=>$data,'page'=>$page);
	}else{
		$json = array('code'=>$result,'message'=>$message,'data'=>$data);
	}
...
function returnJson($result,$message,$data='',$page=''){
	if(!empty($page)){
		$json = array('code'=>$result,'message'=>$message,'data'=>$data,'page'=>$page);
	}else{
		$json = array('code'=>$result,'message'=>$message,'data'=>$data);
