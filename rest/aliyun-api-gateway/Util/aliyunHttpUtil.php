...
class aliyunHttpUtil
{
    ...
    {
		$response = new aliyunHttpResponse();
	...
	private static function InitHttpRequest($host, $path, $method, $readtimeout, $connectTimeout, $headers, $querys)
	{
		...
		if (1 == strpos("$".$host, aliyunHttpSchema::HTTPS))
		{
	...
	private static function InitialBasicHeader($path, $appKey, $appSecret, $method, $headers, $querys, $bodys, $signHeaderPrefixList)
	{
		...
		$headers[aliyunSystemHeader::X_CA_TIMESTAMP] = strval(time()*1000);
		//防重放，协议层不能进行重试，否则会报NONCE被使用；如果需要协议层重试，请注释此行
		$headers[aliyunSystemHeader::X_CA_NONCE] = strval(self::NewGuid());

		$headers[aliyunSystemHeader::X_CA_KEY] = $appKey;
		$headers[aliyunSystemHeader::X_CA_SIGNATURE] = aliyunSignUtil::Sign($path, $method, $appSecret, $headers, $querys, $bodys, $signHeaderPrefixList);
