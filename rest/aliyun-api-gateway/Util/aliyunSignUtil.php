...
class aliyunSignUtil
{
	...
	private static function BuildStringToSign($path, $method, &$headers, $querys, $bodys, $signHeaderPrefixList)
    {
		...
		$sb.= aliyunConstants::LF;
		if (array_key_exists(aliyunHttpHeader::HTTP_HEADER_ACCEPT, $headers) && null != $headers[aliyunHttpHeader::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[aliyunHttpHeader::HTTP_HEADER_ACCEPT];
		}
		$sb.= aliyunConstants::LF;
		if (array_key_exists(aliyunHttpHeader::HTTP_HEADER_CONTENT_MD5, $headers) && null != $headers[aliyunHttpHeader::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[aliyunHttpHeader::HTTP_HEADER_CONTENT_MD5];
		}
		$sb.= aliyunConstants::LF;
		if (array_key_exists(aliyunHttpHeader::HTTP_HEADER_CONTENT_TYPE, $headers) && null != $headers[aliyunHttpHeader::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[aliyunHttpHeader::HTTP_HEADER_CONTENT_TYPE];
		}
		$sb.= aliyunConstants::LF;
		if (array_key_exists(aliyunHttpHeader::HTTP_HEADER_DATE, $headers) && null != $headers[aliyunHttpHeader::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[aliyunHttpHeader::HTTP_HEADER_DATE];
		}
		$sb.= aliyunConstants::LF;
    ...
    */
    private static function BuildHeaders(&$headers, $signHeaderPrefixList)
	{
		...
		if (null != $signHeaderPrefixList)
		{
            ...
            unset($signHeaderPrefixList[aliyunSystemHeader::X_CA_SIGNATURE]);
			unset($signHeaderPrefixList[aliyunHttpHeader::HTTP_HEADER_ACCEPT]);
			unset($signHeaderPrefixList[aliyunHttpHeader::HTTP_HEADER_CONTENT_MD5]);
			unset($signHeaderPrefixList[aliyunHttpHeader::HTTP_HEADER_CONTENT_TYPE]);
			unset($signHeaderPrefixList[aliyunHttpHeader::HTTP_HEADER_DATE]);
			...
			if (is_array($headers)) {
				...
				foreach ($headers as $itemKey => $itemValue)
				{
					if (self::IsHeaderToSign($itemKey, $signHeaderPrefixList))
                    {
                        ...
                        {
                            $signHeadersStringBuilder.= aliyunConstants::SPE1;
                        }
                        $signHeadersStringBuilder.= $itemKey;
                    }
				}
				$headers[aliyunSystemHeader::X_CA_SIGNATURE_HEADERS] = $signHeadersStringBuilder;
			}
    ...
    */
    private static function IsHeaderToSign($headerName, $signHeaderPrefixList)
	{
		...
		if (1 == strpos("$".$headerName, aliyunConstants::CA_HEADER_TO_SIGN_PREFIX_SYSTEM)) {
			return true;
		}
